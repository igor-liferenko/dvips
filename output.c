...
pageinit(int mypageseq)
{
   ...
   if (!disablecomments) {
      if (multiplesects)
         ...
         fprintf(bitfile, "%%%%Page: %d %d\n", pagenum, (mysepfiles&&(pagelist||notfirst||notlast)) ? mypageseq : thispage);
#endif
   }
   linepos = 0;
   cmdout("TeXDict");
   ...
   numout((mysepfiles&&(pagelist||notfirst||notlast)) ? mypageseq-1 : (integer)thispage-1);
   cmdout("bop");
   d = 0;
}
